...
public class ListingToOrderServiceImpl implements ListingToOrderService {
    private final String LISTING_API_BASE_URL = "http://34.87.180.11/Buyer/listings/get/";
    ...
    public List<ListingDTO> getListingsByOrderId(UUID orderId) {
        ...
        if (order.isPresent()) {
            ...
            for (ListingToOrder listingToOrder : listOfListingToOrder) {
                ...
                ListingDTO listingDTO = new ListingDTO(listing.getListingId(), listing.getName(), listing.getPrice(), listing.getSeller(), listingToOrder.getQuantity());
